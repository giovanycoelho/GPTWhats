...
const createWindow = () => {
  ...
  waitForServer(() => {
    ...
    mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
      ...
      console.log('Retrying to load:', appURL);
      // Retry after a delay
      setTimeout(() => {
        mainWindow.loadURL(appURL);
      }, 2000);
...
const startServer = () => {
  // Don't start server when using start:electron script 
  // (server should already be running or will be started by the script)
  console.log('Electron mode: server should already be running on localhost:3001');
  return;
